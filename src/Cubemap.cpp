...
namespace Aura {
	Cubemap::Cubemap(Device& device, bool nearestFilter)
		: m_device(device), m_nearestFilter(nearestFilter)
	...
	void Cubemap::LoadTexture(const std::vector<std::string>& fileNames)
	{
		CreateTextureImage(fileNames);
	...
	void Cubemap::CreateTextureImage(const std::vector<std::string>& fileNames)
	{
		...
		for (int i = 0; i < NUMBER_OF_CUBEMAP_IMAGES; i++) {
			stbi_uc* pixels = stbi_load(fileNames[i].c_str(), &m_width, &m_height, &m_texChannels, STBI_rgb_alpha);
			if (!pixels)
			{
			...
			if (i == 0)
			{
				...
				m_stagingBuffer = std::make_unique<Buffer>(m_device, imageSize, 1, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
			...
			m_stagingBuffer->WriteToBuffer((void*)pixels, imageSize);
